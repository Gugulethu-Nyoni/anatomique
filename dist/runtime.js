...
export function bindText(node, derivedState) {
    ...
    return cleanup; // Return cleanup for external management
}

export function bindAttr(element, attrName, derivedState) {
    // For boolean attributes (e.g., disabled, checked)
    // Note: The previous logic for boolean vs non-boolean attributes
    // was slightly redundant if derivedState.value itself is used.
...
export function bindClass(element, className, derivedCondition) {
    // derivedCondition is expected to be a $derived object
    const cleanup = $effect(() => {
    ...
    // Return cleanup function to be collected by the parent block/component cleanup
    return cleanup;
}
